defmodule WebsockexAdapter.ClientSupervisor do
  @moduledoc """
  ...
  """
  @spec start_client(String.t() | WebsockexAdapter.Config.t(), keyword()) ::
          {:ok, WebsockexAdapter.Client.t()} | {:error, term()}
  def start_client(url_or_config, opts \\ []) do
    ...
    child_spec = %{
      id: make_ref(),
      start: {WebsockexAdapter.Client, :start_link, [url_or_config, supervised_opts]},
      restart: :transient,
      type: :worker
    }
        ...
        try do
          case GenServer.call(pid, :await_connection, timeout) do
            {:ok, state} ->
              ...
              client = %WebsockexAdapter.Client{
