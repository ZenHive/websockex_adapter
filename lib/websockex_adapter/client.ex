defmodule WebsockexAdapter.Client do
  ...
  alias WebsockexAdapter.Helpers.Deribit
  ...
  """
  @spec start_link(String.t() | WebsockexAdapter.Config.t(), keyword()) ::
          {:ok, pid()} | {:error, term()}
  def start_link(url_or_config, opts \\ []) do
    config =
      case url_or_config do
        url when is_binary(url) ->
          case WebsockexAdapter.Config.new(url, opts) do
            {:ok, config} -> config
            {:error, reason} -> {:error, reason}
          end

        %WebsockexAdapter.Config{} = config ->
          config
      end
    ...
    case config do
      ...
      %WebsockexAdapter.Config{} = valid_config ->
        GenServer.start_link(__MODULE__, {valid_config, opts})
    end
  end

  @spec connect(String.t() | WebsockexAdapter.Config.t(), keyword()) :: {:ok, t()} | {:error, term()}
  ...
  def reconnect(%__MODULE__{url: url} = client) do
    ...
    case connect(url) do
      ...
      {:error, reason} ->
        if WebsockexAdapter.ErrorHandler.recoverable?(reason) do
  ...
  @impl true
  def init({%WebsockexAdapter.Config{} = config, opts}) do
    # Setup message handler callback
    handler = Keyword.get(opts, :handler, &WebsockexAdapter.MessageHandler.default_handler/1)
  ...
  def handle_continue(:connect, %{config: config} = state) do
    ...
    case WebsockexAdapter.Reconnection.establish_connection(config) do
  ...
  def handle_continue(:reconnect, %{config: config} = state) do
    ...
    case WebsockexAdapter.Reconnection.establish_connection(config) do
      ...
      {:error, reason} ->
        ...
        retry_delay =
          WebsockexAdapter.Reconnection.calculate_backoff(
            current_attempt,
            config.retry_delay,
            config.max_backoff
          )
  ...
  def handle_info(:retry_reconnect, %{config: config} = state) do
    ...
    if WebsockexAdapter.Reconnection.max_retries_exceeded?(current_retries, config.retry_count) do
  ...
  defp handle_connection_error(state, reason) do
    ...
    if state.config.reconnect_on_error && WebsockexAdapter.Reconnection.should_reconnect?(reason) do
