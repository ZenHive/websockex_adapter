defmodule WebsockexAdapter do
  @moduledoc """
  WebsockexAdapter is a robust WebSocket client library for Elixir with a pluggable adapter architecture.

  ## Architecture Overview

  WebsockexAdapter employs a "thin adapter" architecture that separates concerns through:
  ...
  * **Platform Adapters**: Thin adapters for specific WebSocket services
  ...
  {:ok, conn} = WebsockexAdapter.Connection.start_link(
    adapter: WebsockexAdapter.Platform.Echo.Adapter
  )

  # Send a message and get the response
  {:text, response} = WebsockexAdapter.Client.send_text(conn, "Hello")
  ```
  ...
  defmodule MyApp.WebSocketClient do
    ...
    def handle_call({:send, message}, _from, %{conn: conn} = state) do
      result = WebsockexAdapter.Client.send_text(conn, message)
      {:reply, result, state}
    end
  end
  ```
  ...
  the behaviors in `WebsockexAdapter.Behaviors`:

  ```elixir
  defmodule MyApp.MessageHandler do
    @behaviour WebsockexAdapter.Behaviors.MessageHandler
  ...
  To support a new WebSocket service, implement the `WebsockexAdapter.Platform.Adapter` behavior:

  ```elixir
  defmodule MyApp.CustomAdapter do
    use WebsockexAdapter.Platform.Adapter,
  ...
  * `MetricsCollector`: Collect metrics about WebSocket operations
  ...
  Each behavior has a corresponding default implementation in the `WebsockexAdapter.Defaults` namespace.
  """
end
