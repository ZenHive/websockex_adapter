defmodule WebsockexAdapter.Examples.SupervisedConnectionTest do
  use ExUnit.Case, async: false
  alias WebsockexAdapter.{Client, ClientSupervisor}
  ...
  describe "error handling and recovery" do
    ...
    test "supervised client maintains state across restarts" do
      ...
      case Client.subscribe(client, ["test.channel"]) do
        :ok -> :ok
        {:ok, _} -> :ok
        {:error, _} -> :ok  # Echo server doesn't support subscriptions
      end
  ...
  describe "advanced supervision patterns" do
    ...
    test "multiple supervised connections with different configs" do
      ...
      clients = for config <- configs do
        {:ok, client} = ClientSupervisor.start_client(config.url, 
          Keyword.new(Map.drop(config, [:url]))
        )
        client
      end
    ...
    test "connection manager pattern" do
      defmodule ConnectionManager do
        ...
        def handle_call({:get_client, url}, _from, state) do
          client = case List.keyfind(state.clients, url, 0) do
            {^url, client} -> client
            nil -> nil
          end
          {:reply, client, state}
